...
CONFIG = ConfigManager(
    ...
    [
        ...
        # Looks in env files in order specified
        ConfigEnvFileEnv([
            os.environ.get('ERRBOT_ENV'),
            path.join(path.dirname(__file__), 'errbot_env'),
            '~/.errbot_env',
        ]),
    ],
    ...
    # docs
    doc='Check http://errbot.io/en/latest/user_guide/setup.html#configuration for docs'
)
...
BACKEND = CONFIG('ERRBOT_BACKEND')  # Errbot will start in text mode (console only mode) and will answer commands from there.
...
BASE_DIR = CONFIG('ERRBOT_BASE_DIR')

# The location where all of Err's data should be stored. Make sure to set
# this to a directory that is writable by the user running the bot.
BOT_DATA_DIR = path.join(BASE_DIR, 'data')
...
BOT_EXTRA_PLUGIN_DIR = path.join(BASE_DIR, 'plugins')
...
debug = CONFIG('ERRBOT_DEBUG', default='False', parser=bool)
BOT_LOG_LEVEL = logging.DEBUG if debug else logging.INFO
...
BOT_IDENTITY = {
    ...
    # 'token': CONFIG('ERRBOT_SLACK_TOKEN')

    ## Telegram mode (comment the others above if using this mode)
    # 'token': CONFIG('ERRBOT_TELEGRAM_TOKEN')
